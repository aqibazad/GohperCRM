...
namespace LegalAndGeneralConsultantCRM.Areas.Admin.Controllers
{
    ...
    public class ReportsController : Controller
    {
        ...
        public async Task<IActionResult> CityViseReport()
        {
           
            return View();
        }
        public JsonResult GetCityViseLead(string city)
        {
            var leadsInCity = _context.Leads
                .Where(lead => lead.City == city)
                .Select(lead => new
                {
            ...
            return Json(new { data = leadsInCity });
        }
        public async Task<IActionResult> PrintCityReport(string city)
        {
            // Fetch the leads associated with the specified city
            var leadsInCity = _context.Leads
               
                .Where(lead => lead.City == city)
                .ToList();

            var result = leadsInCity.Select(lead => new
            {
            ...
            var report = GenerateLeadCityReports(dataTable, "CityLeadsReport", "");
        ...
        public IActionResult GenerateLeadCityReports(DataTable tab, string reportname, string export = "pdf")
        {
            ...
            localReport.AddDataSource("dsCityLeads", tab);
